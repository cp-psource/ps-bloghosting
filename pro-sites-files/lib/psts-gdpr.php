...
class ProSites_GDPR {
	...
	public function add_privacy_policy_content() {
		...
		// Content.
		$content = __( 'Einige Details zu Deinen registrierten Webseiten, dem Zahlungsgateway (wie Abonnement-ID, Plan-ID usw.) werden in Bloghosting-Datenbanktabellen und Usermeta gespeichert.', 'psts' );
	...
	public function add_privacy_policy_confirmation( $errors ) {
		 ...
		 */
		$content .= apply_filters( 'psts_privacy_check_text', sprintf( __( 'Ich habe die %sDatenschutz-Bestimmungen%s gelesen und akzeptiert', 'psts' ), '<a href="' . $privacy_link . '" target="_blank">', '</a>' ) );
	...
	public function check_privacy_policy_confirmation( $result = array() ) {
		 ...
		 */
		$error_text = apply_filters( 'psts_privacy_check_error_text', __( 'Du musst die Datenschutz und Richtlinien lesen und akzeptieren.', 'psts' ) );
	...
	public function register_exporter( $exporters = array() ) {
		...
		$exporters['psts'] = array(
			'exporter_friendly_name' => __( 'Bloghosting', 'psts' ),
	...
	public function register_eraser( $erasers = array() ) {
		...
		$erasers['psts'] = array(
			'eraser_friendly_name' => __( 'Bloghosting', 'psts' ),
	...
	public function remove_personal_data( $email_address ) {
		...
		return array(
			...
			'messages'       => array( __( 'Der Benutzer wurde von allen Webseiten entfernt.', 'psts' ) ),
			'done'           => true,
		);
	}
	...
	public function user_data( $user_id ) {
		...
		foreach ( $user_blogs as $blog_id => $blog ) {
			...
			$blog_data = array(
				array(
					'name'  => __( 'Seitentitel', 'psts' ),
					'value' => $blog->blogname,
				),
				array(
					'name'  => __( 'Seiten URL', 'psts' ),
					'value' => $blog->siteurl,
				),
			);
	...
	private function set_stripe_data( $blog_id, &$blog_data ) {
		...
		if ( ! empty( $result ) ) {
			// Customer data.
			$blog_data[] = array(
				'name'  => __( 'Kunden-ID', 'psts' ),
				'value' => $result->customer_id,
			);
			// Subscription data.
			$blog_data[] = array(
				'name'  => __( 'Abonnement-ID', 'psts' ),
