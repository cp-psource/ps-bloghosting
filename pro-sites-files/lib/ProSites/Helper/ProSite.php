	...
	class ProSites_Helper_ProSite {
		...
		public static function get_blog_info( $blog_id ) {
			...
			if ( $trialing ) {
				...
				$trial_message = ( 'manual' === $gateway || ! $is_recurring ) ? __( 'You are still within your trial period.', 'psts' ) : __( 'Du befindest Dich noch in Deiner Testphase. Sobald Deine Testphase abgeschlossen ist, wird Dein Konto automatisch belastet.', 'psts' );
			...
			if ( $is_recurring && ! $psts->is_blog_canceled( $blog_id ) ) {
				$cancel_info_message = '<p class="prosites-cancel-description">' . sprintf( __( 'Wenn Du Dein Abonnement kündigen möchtest, sollte diese Webseite weiterhin %1$s Funktionen bis %2$s haben.', 'psts' ), $level, $end_date ) . '</p>';
