...
if ( ! class_exists( 'ProSites_Module_Taxamo' ) ) {
	...
	class ProSites_Module_Taxamo {
		...
		public static function init() {
			...
			add_filter( 'prosite_checkout_tax_apply', array( self::class, 'apply_tax' ), 10, 4 );
			add_filter( 'prosite_checkout_tax_percentage', array( self::class, 'tax_percentage' ), 10, 4 );

			add_filter( 'prosites_get_tax_object', array( self::class, 'get_tax_object' ) );
			add_filter( 'prosites_get_tax_evidence_string', array( self::class, 'get_evidence_string' ), 10, 2 );
			add_filter( 'prosites_tax_evidence_from_json_data', array( self::class, 'get_evidence_from_json_data' ), 10, 2 );
			add_filter( 'prosites_tax_country_from_data', array( self::class, 'get_country_from_data' ), 10, 3 );
			add_filter( 'prosites_tax_ip_from_data', array( self::class, 'get_ip_from_data' ), 10, 3 );

			add_action( 'prosites_transaction_record', array( self::class, 'record_transaction' ) );
