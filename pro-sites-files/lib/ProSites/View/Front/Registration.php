	...
	class ProSites_View_Front_Registration {
		...
		public static function render_signup_form( $render_data = array(), $errors = false ) {
			...
			if ( 'sign_up' == $action || 'another_blog' == $action ) {
				...
				$content .= __( 'Aktualisieren der verfügbaren Vorlagen für den Plan...', 'psts' );
				$content .= '</div><br/>';
				...
				$content .= '<h2>' . esc_html__( 'Richte Deine Webseite ein', 'psts' ) . '</h2>';
				...
				$content .= '<div><input type="button" id="check-prosite-blog" value="' . esc_attr__( 'Reserviere Deine Website', 'psts' ) . '" /></div>';
		...
		private static function render_user_section( $render_data = array(), $errors, $user_name, $user_email ) {
			...
			if( ! is_user_logged_in() ) {
				...
				$content .= '<div class="username"><label for="user_name">' . __( 'Benutzername:' ) . '</label>';
				...
				$content .= __( '(Muss aus min 4 Zeichen, Kleinbuchstaben oder Zahlen bestehen.)', 'psts' );
				$content .= '</div>';

				$content .= '<div class="email"><label for="user_email">' . __( 'Email&nbsp;Addresse:', 'psts' ) . '</label>';
				...
				$content .= __('Wir senden Deine Registrierungs-E-Mail an diese Adresse. (Überprüfe Deine E-Mail-Adresse, bevor Du fortfährst.)');
				$content .= '</div>';
...
//			if ( !is_subdomain_install() ) {
			$content .= '<div class="blogname"><label for="blogname">' . __('Deine Webseite: ', 'psts') . '</label>';
//			} else {
...
//			}
			...
			$content .= '<div class="blog_title"><label for="blog_title">' . esc_html__('Webseitentitel:', 'psts' ) . '</label>';
			...
			$content .= '<div id="privacy">
        		<p class="privacy-intro">
            		<label for="blog_public_on">' . esc_html__('Privatsphäre:', 'psts') . '</label> ' .
			            esc_html__( 'Suchmaschinen erlauben diese Seite zu katalogisieren.', 'psts' ) .
			            '<br style="clear:both" />
	                <label class="checkbox" for="blog_public_on">
		                <input type="radio" id="blog_public_on" name="blog_public" value="1" ' . $yes_checked  . '/>
		                <strong>' . esc_html__( 'Ja', 'psts' ) . '</strong>
	                </label>
	                <label class="checkbox" for="blog_public_off">
		                <input type="radio" id="blog_public_off" name="blog_public" value="0" ' . $no_checked  . '/>
		                <strong>' . esc_html__( 'Nein', 'psts' ) . '</strong>
	                </label>
        		</p>
			</div>';
