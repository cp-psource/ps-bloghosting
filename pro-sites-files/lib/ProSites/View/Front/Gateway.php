...
//				}
			...
			if ( $tabbed && $available_gateways > 1 ) {
				...
				if ( ! empty( $primary_gateway ) ) {
					...
					$content .= '<li class="' . $class . '"><a href="#gateways-1">' . esc_html( $psts->get_setting( 'checkout_gateway_primary_label', __( 'Zahlung', 'psts' ) ) ) . '</a></li>';
				}
				if ( ! empty( $secondary_gateway ) ) {
					//If there are errors associated with the gateway, Mark the gateway tab as active
					$error = $psts->errors->get_error_message( $secondary_gateway );
					$class = ! empty( $error ) ? 'ui-tabs-active ui-state-active' : '';
					$content .= '<li class="' . $class . '"><a href="#gateways-2">' . esc_html( $psts->get_setting( 'checkout_gateway_secondary_label', __( 'Alternative Zahlung', 'psts' ) ) ) . '</a></li>';
				}
				if ( ! empty( $manual_gateway ) ) {
					//If there are errors associated with the gateway, Mark the gateway tab as active
					$error = $psts->errors->get_error_message( $manual_gateway );
					$class = ! empty( $error ) ? 'ui-tabs-active ui-state-active' : '';
					$content .= '<li class="' . $class . '"><a href="#gateways-3">' . esc_html( $psts->get_setting( 'checkout_gateway_manual_label', __( 'Offline-Zahlung', 'psts' ) ) ) . '</a></li>';
				}
				$content .= '</ul>';
			}
				...
				if ( $allow_cancel_gateway && ! empty ( $last_gateway ) && $last_gateway !== $primary_gateway && strtolower( $last_gateway ) !== 'trial' ) {
					...
					if ( ! empty( $last_gateway ) && 'manual' != $last_gateway ) {
						$content .= '<div id="psts-general-error" class="psts-warning">' .
						            wp_kses( sprintf( __( 'Du hast Dich beim <strong>%1$s</strong> Zahlungsgateway angemeldet. Wir werden versuchen, Deine <strong>%1$s</strong> Zahlungen zu stornieren und neue Zahlungen einzurichten, wenn Du fortfahren möchtest.', 'psts' ), esc_html( $name ) ), $allowed_html );
						$content .= '</div>';
					}
...
//				if( ! empty ( $site_details['last_payment_gateway'] ) && $site_details['last_payment_gateway'] !== $secondary_gateway ) {
					...
					if ( ! empty( $last_gateway ) && 'manual' != $last_gateway ) {
						$content .= '<div id="psts-general-error" class="psts-warning">' .
						            sprintf( wp_kses( __( 'Du hast Dich beim <strong>%1$s</strong> Zahlungsgateway angemeldet. Wir werden versuchen, Deine <strong>%1$s</strong> Zahlungen zu stornieren und neue Zahlungen einzurichten, wenn Du fortfahren möchtest.', 'psts' ), $allowed_html ), esc_html( $name ) );
						$content .= '</div>';
					}
		...
		public static function render_current_plan_information( $render_data = array(), $blog_id, $domain, $gateways, $gateway_order ) {
			...
			if ( ! empty( $info_retrieved ) && empty( $info_retrieved['complete_message'] ) ) {
				...
				if ( $info_retrieved['recurring'] && ! empty( $info_retrieved['last_payment_gateway'] ) && 'manual' == strtolower( $info_retrieved['last_payment_gateway'] ) ) {
					$content .= '<div id="psts-general-error" class="psts-warning psts-manual-payment-notify">' . __( 'Deine Websrite wurde manuell erweitert und möglicherweise nicht automatisch erneuert. Wenn Du ein aktives Abonnement hast, verschwindet diese Nachricht, sobald die Zahlung eingegangen ist. Bitte kontaktiere uns, wenn Du Bedenken hast. ', 'psts' ) . '</div>';
				}
				...
				if ( ! empty( $info_retrieved['payment_failed'] ) ) {
					if ( ! empty( $info_retrieved['last_payment_gateway'] ) && 'stripe' === strtolower( $info_retrieved['last_payment_gateway'] ) ) {
						$content .= '<div id="psts-general-error" class="psts-warning psts-payment-failed">' . __( 'Deine letzte Zahlung ist fehlgeschlagen. Bitte aktualisiere Deine Karte unten oder wähle eine neue Zahlungsoption.', 'psts' ) . '</div>';
					} else {
						$content .= '<div id="psts-general-error" class="psts-warning psts-payment-failed">' . __( 'Deine letzte Zahlung ist fehlgeschlagen. Bitte wähle unten Deinen Plan aus und wähle eine neue Zahlungsoption.', 'psts' ) . '</div>';
					}
				}
				...
				if ( ! empty( $info_retrieved['card_type'] ) ) {
					$content .= '<li class="psts-payment-method">' . sprintf( __( 'Zahlungsmethode: <strong>%1$s Karte</strong> läuft ab <strong>%2$s</strong>. Läuft ab <strong>%3$s/%4$s</strong>', 'psts' ), $info_retrieved['card_type'], $info_retrieved['card_reminder'], $info_retrieved['card_expire_month'], $info_retrieved['card_expire_year'] );
				...
				if ( ! empty( $info_retrieved['last_payment_date'] ) ) {
					$content .= '<li class="psts-last-payment">' . esc_html__( 'Letzter Zahlungstermin:', 'psts' ) . ' <strong>' . date_i18n( get_option( 'date_format' ), $info_retrieved['last_payment_date'] ) . '</strong></li>';
				}
				//next payment
				if ( ! empty( $info_retrieved['next_payment_date'] ) ) {
					$content .= '<li class="psts-next-payment">' . esc_html__( 'Nächster Zahlungstermin:', 'psts' ) . ' <strong>' . date_i18n( get_option( 'date_format' ), $info_retrieved['next_payment_date'] ) . '</strong></li>';
				}
				//period
				if ( ! empty( $info_retrieved['period'] ) && ! empty( $info_retrieved['recurring'] ) ) {
					$content .= '<li class="psts-period">' . esc_html__( 'Verlängerungszeitraum:', 'psts' ) . sprintf( __( ' Je <strong>%d</strong> Monat(e)', 'psts' ), $info_retrieved['period'] ) . '</li>';
				}
				// Is recurring?
				if ( empty( $info_retrieved['recurring'] ) ) {
					$content .= '<li class="psts-expiry">' . esc_html__( 'Plan läuft ab am:', 'psts' ) . ' <strong>' . $info_retrieved['expires'] . '</strong></li>';
				} else if ( ! empty( $info_retrieved['recurring'] ) && empty( $info_retrieved['next_payment_date'] ) ) {
					$content .= '<li class="psts-expiry">' . esc_html__( 'Verlängerung fällig am:', 'psts' ) . ' <strong>' . $info_retrieved['expires'] . '</strong></li>';
				}
			...
			return '<div id="psts_existing_info"><h2>' . esc_html__( 'Dein aktueller Plan', 'psts' ) . '</h2>' . $content . '</div>';
		...
		public static function prepend_plan_details( $content, $blog_id, $domain ) {
			...
			if ( $is_pro_site && ( $cancel || ! isset( $_GET['action'] ) || $page_reload || $new_blog_allowed ) ) {
				...
				if ( isset( $gateways ) && isset( $gateway_details ) ) {
					...
					$plan_content .= '<h2>' . esc_html__( 'Ändere Deinen Plan', 'psts' ) . '</h2>';
				}
			} else {
				// NOTIFICATIONS ONLY
		...
		public static function render_manual_submitted( $render_data = array() ) {
			...
			$content .= '<h2>' . esc_html__( 'Finalisiere Deine Webseite...', 'psts' ) . '</h2>';

			$content .= '<p>' . esc_html__( 'Vielen Dank, dass Du eine Anfrage zur manuellen Zahlung gesendet hast. Wir werden Deine Informationen überprüfen und Dir eine E-Mail senden, sobald Deine Webseite aktiviert oder erweitert wurde.', 'psts' ) . '</p>';

			$content .= '<p>' . sprintf( esc_html__( 'Die E-Mail-Adresse, die wir von Dir haben, lautet: %s', 'psts' ), $email ) . '</p>';
		...
		public static function render_free_confirmation( $render_data = array() ) {
			...
			$content .= '<p>' . esc_html__( 'Deine Basis-Seite wurde eingerichtet und Du solltest bald eine E-Mail mit Deinen Webseiten-Details und Deinem Passwort erhalten. Du kannst Deine Webseite jederzeit aktualisieren, indem Du Dich anmeldest und Dein Konto anzeigst.', 'psts' ) . '</p>';

			$content .= '<p><strong>' . esc_html__( 'Deine Anmeldedaten sind:', 'psts' ) . '</strong></p>';
			$content .= '<p>' . sprintf( esc_html__( 'Benutzername: %s', 'psts' ), $username );
			$content .= '<br />' . esc_html__( 'Administrations URL: ', 'psts' ) . '<a href="' . esc_url( $blog_admin_url ) . '">' . esc_html( $blog_admin_url ) . '</a></p>';

			$content .= '<p>' . esc_html__( 'Wenn Du keine E-Mail erhalten hast, versuche bitte Folgendes:', 'psts' ) . '</p>';
			$content .= '<ul>' .
			            '<li>' . esc_html__( 'Warte etwas länger.', 'psts' ) . '</li>' .
			            '<li>' . esc_html__( 'Überprüfe Deinen Spam-Ordner für den Fall, dass sie dort landet.', 'psts' ) . '</li>' .
			            '<li>' . sprintf( esc_html__( 'Stelle sicher, dass Deine E-Mail-Adresse korrekt ist (%s)', 'psts' ), $email ) . '</li>' .
			            '</ul>';
			$content .= '<p>' . esc_html__( 'Wenn Deine E-Mail-Adresse falsch ist oder Du ein Problem festgestellt hast, kontaktiere uns bitte, um das Problem zu beheben.', 'psts' ) . '</p>';

			if ( ! empty( $blog_admin_url ) && ! is_user_logged_in() ) {
				$content .= '<a class="button" href="' . esc_url( $blog_admin_url ) . '">' . esc_html__( 'Jetzt einloggen', 'psts' ) . '</a>';
			}
		...
		public static function render_payment_submitted( $render_data = array(), $show_trial = false, $blog_id = '' ) {
			...
			if ( ! $show_trial ) {
				$content .= '<p>' . esc_html__( 'Deine Zahlung wird verarbeitet und Du solltest bald eine E-Mail mit Deinen Webseiten-Details erhalten.', 'psts' ) . '</p>';
			} else {
				$content .= '<p>' . esc_html__( 'Deine Webseiten-Testphase wurde eingerichtet und Du solltest bald eine E-Mail mit Deinen Webseiten-Details erhalten. Sobald Deine Testversion abgeschlossen ist, wirst Du aufgefordert, ein manuelles Upgrade durchzuführen.', 'psts' ) . '</p>';
			}
			...
			if ( ! empty( $userpass ) ) {
				$content .= '<br />' . sprintf( esc_html__( 'Passwort: %s', 'psts' ), $userpass );
			}
			$content .= '<br />' . esc_html__( 'Administrations URL: ', 'psts' ) . '<a href="' . esc_url( $blog_admin_url ) . '">' . esc_html( $blog_admin_url ) . '</a></p>';
		...
		public static function render_account_modified( $content, $blog_id, $domain ) {
			...
			$content .= '<h2>' . esc_html__( 'Plan aktualisiert...', 'psts' ) . '</h2>';
			$content .= '<p>' . sprintf( esc_html__( 'Dein Plan wurde erfolgreich von %s auf %s geändert. Du erhältst in Kürze eine Empfangs-E-Mail, um diese Aktion zu bestätigen.', 'psts' ), $previous, $current ) . '</p>';
			$content .= '<p>' . esc_html__( 'Wenn Du keine E-Mail erhalten hast, versuche bitte Folgendes:', 'psts' ) . '</p>';
			$content .= '<ul>' .
			            '<li>' . esc_html__( 'Warte etwas länger.', 'psts' ) . '</li>' .
			            '<li>' . esc_html__( 'Überprüfe Deinen Spam-Ordner für den Fall, dass sie dort landet.', 'psts' ) . '</li>' .
			            ...
			            '</ul>';
			$content .= '<p>' . esc_html__( 'Wenn Deine E-Mail-Adresse falsch ist oder Du ein Problem festgestellt hast, kontaktiere uns bitte, um das Problem zu beheben.', 'psts' ) . '</p>';
			$content .= '<a href="' . $psts->checkout_url( $blog_id ) . '">' . esc_html__( 'Gehe zurück zu Deinem Konto.', 'psts' ) . '</a>';
			$content .= '</div>';
