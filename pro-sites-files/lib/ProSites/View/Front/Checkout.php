	...
	class ProSites_View_Front_Checkout {
				...
				if ( $show_buy_buttons ) {
					if ( empty( $column['button'] ) ) {
						if ( $add_coupon && 'option1' == $psts->get_setting( 'pricing_table_coupon_position', 'option1' ) ) {
							...
							$content .= '<input type="text" name="apply-coupon" placeholder="' . __( 'Gutschein eingeben', 'psts' ) . '" />';
		...
		public static function get_pricing_columns( $show_header, $show_features, $show_buy_button = true ) {
			...
			if ( $show_features && $show_table && ( count( $enabled_modules ) > 0 || ! empty( $feature_table['feature_order'] ) ) ) {
				...
				$columns[ $col_count ]['sub_title'] = __( 'Vergleiche Features', 'psts' );
			...
			if ( $show_buy_button ) {
				...
				if ( $show_header ) {
					...
					foreach ( $pricing_levels_order as $level ) {
						if ( ! self::$new_signup ) {
							$columns[ $col_count ]['button'] = '<button class="choose-plan-button">' . __( 'Plan wählen', 'psts' ) . '</button>';
						} else {
							$columns[ $col_count ]['button'] = '<button class="choose-plan-button register-new">' . __( 'Anmelden', 'psts' ) . '</button>';
						}
			...
			if ( $coupons_enabled ) {
				$col_count                       = 0;
				$columns[ $col_count ]['coupon'] = __( 'Gutschein einlösen', 'psts' );
			}
		...
		private static function get_header_details( $level = false ) {
			...
			$periods = array(
				'price_1'  => __( 'jeden Monat', 'psts' ),
				'price_3'  => __( 'alle 3 Monate', 'psts' ),
				'price_12' => __( 'alle 12 Monate', 'psts' ),
			);

			$periods_non_recurring = array(
				'price_1'  => __( 'für 1 Monat', 'psts' ),
				'price_3'  => __( 'für 3 Monate', 'psts' ),
				'price_12' => __( 'für 12 Monate', 'psts' ),
			);

			$payment_type = array(
				'price_1'  => __( 'Monatlich', 'psts' ),
				'price_3'  => __( 'Vierteljährlich', 'psts' ),
				'price_12' => __( 'Jährlich', 'psts' ),
			);

			$plan_text = apply_filters( 'prosites_pricing_labels', array(
				'payment_type' => __( 'Zahlungsperiode', 'psts' ),
				'setup'        => __( 'Plus eine einmalige %s Einrichtungsgebühr', 'psts' ),
				'summary'      => __( 'Dies entspricht <strong>nur %s monatlich</strong>. ', 'psts' ),
				'saving'       => __( 'Eine Einsparung von <strong>%s</strong> durch Zahlung von %d Monaten im Voraus.', 'psts' ),
				'monthly'      => __( 'Profitiere von <strong>zusätzlichen Einsparungen</strong>, indem Du im Voraus bezahlst.', 'psts' ),
				'monthly_alt'  => __( '<em>Versuch es!</em><br /><span>Du kannst jederzeit problemlos auf einen günstigeren Plan umsteigen.</span>', 'psts' )
			), $level );
		...
		public static function render_login() {
			$content = sprintf( '<div class="login-existing">
				...
				esc_html__( 'Hast Du bereits eine Webseite?', 'psts' ), // Catchphrase
				esc_url( wp_login_url( get_permalink() ) ), // Login URL
				esc_attr__( 'Einloggen', 'psts' ), // Anchor Title
				esc_html__( 'Jetzt einloggen.', 'psts' ), // Anchor Text
		...
		public static function get_login_form() {
			...
			$content = "<p>" . esc_attr__( 'Du musst angemeldet sein, um diesen Blog zu aktualisieren. Bitte melde Dich unten an.', 'psts' ) . "</p>";
		...
		public static function render_free( $style, $blog_id ) {
			...
			if ( ! isset( $_GET['bid'] ) && empty( $blog_id ) && ! isset( $session_data['new_blog_details']['blogname'] ) && ProSites_Helper_ProSite::allow_new_blog() ) {
				...
				if ( ! is_pro_site( $blog_id ) && ProSites_Helper_ProSite::allow_new_blog() ) {
					$free_link = apply_filters( 'prosites_checkout_free_link', '<a class="pblg-checkout-opt" style="width:100%" id="psts-free-option" href="' . get_admin_url( $blog_id, 'index.php?psts_dismiss=1', 'http' ) . '" title="' . __( 'Ablehnen', 'psts' ) . '">' . $psts->get_setting( 'free_msg', __( 'Nein, danke, ich werde vorerst mit einer einfachen Seite fortfahren', 'psts' ) ) . '</a>', $blog_id );
					$content   = '<div class="free-plan-link-logged-in" style="' . esc_attr( $style ) . '"><p>' . esc_html__( 'Deine aktuelle Seite ist eine einfache Webseite ohne zusätzliche Funktionen. Erweitere sie jetzt, indem Du oben einen Plan auswählst.', 'psts' ) . '</p><p>' . $free_link . '</p></div>';
				}
			}
		...
		private static function render_pricing_grid( $columns, $blog_id = false, $echo = false ) {
			...
			if ( $add_coupon ) {
				...
				$content .= '<div class="coupon-box">';
				$content .= '<input type="text" name="apply-coupon" placeholder="' . __( 'Gutschein eingeben', 'psts' ) . '" />';
				$content .= '<a name="apply-coupon-link" class="apply-coupon-link">' . esc_html__( 'Gutschein einlösen', 'psts' ) . '</a>';
				$content .= '</div></div></div>';
				$content .= '</td>';
				$content .= '</tr>';
			}
		...
		public static function render_standalone_coupon( $content ) {
			...
			<div class="coupon-wrapper">
				<div class="coupon-box post-table">
					<span><input type="text" name="apply-coupon" placeholder="' . esc_attr__( 'Gutschein-Code eingeben', 'psts' ) . '" /></span>
					<button name="apply-coupon-link" class="apply-coupon-link">' . esc_html__( 'Gutschein einlösen', 'psts' ) . '</button>
				</div>
			</div>
			' . $content;
