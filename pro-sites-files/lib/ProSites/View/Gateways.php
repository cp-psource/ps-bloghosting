    ...
    class ProSites_View_Gateways {
		...
		public static function render_tab_gateway_prefs() {
			...
			<table class="form-table">
				<tr>
					<th scope="row"><?php _e( 'Aktive Gateways', 'psts' ) ?></th>
					<td>
						<?php
				...
				<tr>
					<th scope="row"><?php _e( 'Primäres Gateway', 'psts' ); ?></th>
					<td>
						...
						<?php echo '<span>' . __( 'Kassenetikett', 'psts') . '</span>'; ?>
						<input type="text" name="psts[checkout_gateway_primary_label]" value="<?php echo $psts->get_setting( 'checkout_gateway_primary_label', __( 'Zahlung', 'psts' ) ); ?>" />
					</td>
				</tr>
				<tr>
					<th scope="row"><?php _e( 'Sekundäres Gateway', 'psts' ); ?></th>
					<td>
						...
						<?php echo '<span>' . __( 'Kassenetikett', 'psts') . '</span>'; ?>
						<input type="text" name="psts[checkout_gateway_secondary_label]" value="<?php echo $psts->get_setting( 'checkout_gateway_secondary_label', __( 'Alternative Zahlung', 'psts' ) ); ?>" />
					</td>
				</tr>
				<?php
					...
					?>
						<tr>
							<th scope="row"><?php _e( 'Manuelle Gateway-Einstellungen', 'psts' ); ?></th>
							<td>
								<input type="checkbox" name="psts[gateway_pref_use_manual]" <?php checked( $manual_checked, 'on'  ); ?> />
								<?php esc_html_e( 'Verwende das manuelle Gateway als dritte Option.', 'psts' ); ?><br />
								<?php echo '<span>' . __( 'Kassenetikett', 'psts') . '</span>'; ?>
								<input type="text" name="psts[checkout_gateway_manual_label]" value="<?php echo $psts->get_setting( 'checkout_gateway_manual_label', __( 'Offline-Zahlung', 'psts' ) ); ?>" />
							</td>
						</tr>
					<?php
					}
		...
		public static function render_tab_twocheckout() {
			...
			<table class="form-table">
				<tr>
					<th scope="row"><?php _e( 'Gateway aktivieren', 'psts' ) ?></th>
					<td>
		...
		public static function gateway_input_options( $active_gateways, $setting, $allow_none = false ) {
			...
			if( empty( $names ) || ( $allow_none && 1 == count( $names ) ) ) {
				$names = array( 'not_enabled' => __( 'Keine Gateways aktiviert', 'psts' ) );
