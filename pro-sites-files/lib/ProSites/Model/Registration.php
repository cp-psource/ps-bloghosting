...
if ( ! class_exists( 'ProSites_Model_Registration' ) ) {
	...
	class ProSites_Model_Registration {
				...
				if ( ( ! empty( $user_check->errors ) && 1 == count( $user_check->errors ) && ! isset( $_POST['new_blog'] ) ) || ( 0 == count( $user_check->errors ) && isset( $_POST['new_blog'] ) ) ) {
					...
					if ( $keys && ! in_array( 'availability_check_only', $keys ) && ! isset( $_POST['new_blog'] ) ) {
						...
						if ( $trial_active ) {
							...
							if ( $recurring ) {
								$blog_data['new_blog_details']['reserved_message'] = sprintf( '<div class="reserved_msg"><h2>' . __( 'Aktiviere Deine Webseite', 'psts' ) . '</h2>' . __( '<p>Deine Website <strong>(%s)</strong> wurde reserviert, ist jedoch noch nicht aktiviert.</p> <p>Sobald die Zahlungsinformationen überprüft wurden, beginnt Deine Testphase. Wenn Deine Testversion endet, wirst Du automatisch auf den von Dir gewählten Plan aktualisiert. Deine Reservierung dauert nur 48 Stunden. Danach wird Dein Seiten-Name wieder verfügbar.</p><p>Bitte benutze das unten stehende Formular, um Deine Zahlungsinformationen einzurichten.</p>', 'psts' ) . '</div>', $site_name );
							} else {
		...
		public static function prosite_blog_check_only( $email, $user, $errors = false ) {
			if ( empty( $errors ) ) {
				$errors = new WP_Error();
			}

			$errors->add( 'availability_check_only', __( 'Nur Benutzer-/Blog-Verfügbarkeitsprüfung.', 'psts' ) );
		...
		public static function update_nbt_templates() {
			...
			$content = '<option value="none">' . __( "Keine", "psts" ) . '</option>';
			if ( ! empty( $templates ) ) {
