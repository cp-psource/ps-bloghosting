	...
	class ProSites_Model_Pricing {
		...
		public static function process_form() {
			...
			if ( isset( $_POST['submit_pricing'] ) || isset( $_POST['submit_pricing_section'] ) || isset( $_POST['submit_pricing_header'] ) ) {
				...
				echo '<div id="message" class="updated fade"><p>' . __( 'Preistabelleneinstellungen gespeichert!', 'psts' ) . '</p></div>';
					...
					foreach ( $feature['levels'] as $key => $level ) {
						switch ( $type ) {
							case 'module':
								...
								if ( ! empty( $table_settings[ $feature_key ]['levels'][ $key ] ) && is_array( $table_settings[ $feature_key ]['levels'][ $key ]['status'] ) ) {
