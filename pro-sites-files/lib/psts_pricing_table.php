...
class ProSites_Pricing_Table {
	...
	public function show_coupon_feedback( $content ) {
		...
		if ( isset( $coupon_code ) ) {
			...
			$content .= '<div id="psts-coupon-msg">' . sprintf( __( 'Dein Gutscheincode <strong>%1$s</strong> wurde für einen Rabatt von %2$s auf die erste Zahlung angewendet. <a href="%3$s">Entferne es &raquo;</a>', 'psts' ), esc_html( $coupon_code ), $coupon_value['discount'], get_permalink() . "?bid=$blog_id&remove_coupon=1" ) . '</div>';
	...
	private function get_level_description( $level_price, $period = null ) {
		...
		switch ( $period ) {
			case MONTHLY:
				$content = '<p><strong>' . __( 'Versuch es!', 'psts' ) . '</strong></p>';
				if ( in_array( QUARTERLY, $this->periods ) || in_array( QUARTERLY, $this->periods ) ) {
					$content .= '<p class="save">' . __( 'Du kannst jederzeit problemlos zu einem günstigeren Plan wechseln.', 'psts' ) . '</p>';
				}
				$content = apply_filters( 'psts_pricing_plan_description', $content, MONTHLY, $this->periods );
				break;
			case QUARTERLY:
				$content = '<p class="equivalent">' . sprintf( __( 'Entspricht nur <br /> %s monatlich', 'psts' ), $psts->format_currency( false, $level_price [ QUARTERLY ] / 3 ) ) . '</p>';
				if ( in_array( MONTHLY, $this->periods ) && ( ( $level_price [ MONTHLY ] * 3 ) - $level_price [ QUARTERLY ] ) > 0 ) {
					$content .= '<p class="save">' . sprintf( __( 'Spare %s, indem Du 3 Monate im Voraus bezahlst!', 'psts' ), $psts->format_currency( false, ( $level_price [ MONTHLY ] * 3 ) - $level_price [ QUARTERLY ] ) ) . '</p>';
				}
				$content = apply_filters( 'psts_pricing_plan_description', $content, QUARTERLY, $this->periods );
				break;
			case YEARLY:
				$content = '<p class="equivalent">' . sprintf( __( 'Entspricht nur <br /> %s monatlich', 'psts' ), $psts->format_currency( false, $level_price [ YEARLY ] / 12 ) ) . '</p>';
				if ( in_array( MONTHLY, $this->periods ) && ( ( $level_price [ MONTHLY ] * 12 ) - $level_price [ YEARLY ] ) > 0 ) {
					$content .= '<p class="save">' . sprintf( __( 'Spare %s, indem Du ein Jahr im Voraus bezahlst!', 'psts' ), $psts->format_currency( false, ( $level_price [ MONTHLY ] * 12 ) - $level_price [ YEARLY ] ) ) . '</p>';
				}
			...
			if ( is_array( $coupons ) && count( $coupons ) && ! isset ( $coupon_code ) ) {
				$content .= '<div id="psts-coupon-block">
					            ...
					            <div id="psts-coupon-code" style="display: none;">
					                <label for="coupon_code">' . __( 'Gib deinen Code ein:', 'psts' ) . '</label>
					                <input type="text" name="coupon_code" id="coupon_code" />&nbsp;
					                <input type="submit" name="coupon-submit" class="regbutton" value="' . __( 'Anwenden &raquo;', 'psts' ) . '" />
					            </div>
					         </div>';
			}
			$content .= '</div>';
	...
	private function get_plan_content( $period, $data, $level, $level_name ) {
		...
		$choose_plan       = '<a href="#" class="button choose-plan" data-level="' . $level . '" data-level-name="' . strtolower( $level_name ) . '">Wähle einen Plan</a>';
		...
		$content .= '<div class="free-msg-wrapper">';
			if ( get_blog_option( $this->blog_id, 'psts_signed_up' ) && ! apply_filters( 'psts_prevent_dismiss', false ) ) {
				$content .= '<tr class="psts_level level-free">
                    <td valign="middle" class="level-name"><h3>' . $psts->get_setting( 'free_name', __( 'Basisblog', 'psts' ) ) . '</h3></td>';
					$content .= '<td class="level-option" colspan="' . count( $this->periods ) . '">';
					$content .= '<a class="pblg-checkout-opt" style="width:100%" id="psts-free-option" href="' . get_admin_url( $this->blog_id, 'index.php?psts_dismiss=1', 'http' ) . '" title="' . __( 'Ablehnen', 'psts' ) . '">' . $psts->get_setting( 'free_msg', __( 'Nein, danke, ich werde vorerst mit einer einfachen Seite fortfahren', 'psts' ) ) . '</a>';
				$content .= '</td></tr>';
			}
		$content .= '</div>';
