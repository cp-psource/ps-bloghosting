...
class ProSites_Gateway_Manual {
	...
	function settings() {
		...
		<div class="inside">
			<table class="form-table">
				<tr>
					<th scope="row"
					    class="psts-help-div psts-method-name"><?php echo __( 'Methode Label', 'psts' ) . $psts->help_text( __( 'Gib einen öffentlichen Namen für diese Zahlungsmethode ein, der den Benutzern angezeigt wird - Kein HTML', 'psts' ) ); ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"
					    class="psts-help-div psts-user-instruction"><?php echo __( 'Zahlungsanleitung', 'psts' ) . $psts->help_text( __( 'Manuelle Zahlungsanweisungen zur Anzeige auf dem Checkout-Bildschirm - HTML zulässig', 'psts' ) ); ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"
					    class="psts-help-div psts-show-submission"><?php echo __( 'Zeige Einreichungsformular', 'psts' ) . $psts->help_text( __( 'Zeigt einen Textbereich an, in dem der Benutzer Zahlungsdetails eingeben kann. Die Formularübermittlung erfolgt an die E-Mail-Adresse des Netzwerkadministrators.', 'psts' ) ); ?></th>
					<td>
						<label>
				...
				<tr valign="top">
					<th scope="row"
					    class="psts-help-div psts-submission-form-email"><?php echo __( 'Einreichungsformular Email', 'psts' ) . $psts->help_text( __( 'Die E-Mail-Adresse, an die manuelle Übermittlungen des Zahlungsformulars gesendet werden sollen.', 'psts' ) ); ?></th>
					<td>
	...
	function payment_info( $payment_info, $blog_id ) {
		...
		$payment_info .= sprintf( __( 'Nächster Zahlungstermin: %s', 'psts' ), date_i18n( get_option( 'date_format' ), $psts->get_expire( $blog_id ) ) ) . "\n";
	...
	public static function checkout_screen( $content, $blog_id, $domain = '' ) {
		...
		if ( is_pro_site( $blog_id ) ) {
			...
			$content .= '<h3>' . __( 'Deine Kontoinformationen', 'psts' ) . '</h3><ul>';
			$content .= '<li>' . __( 'Level:', 'psts' ) . ' <strong>' . $level . '</strong></li>';

			if ( $old_gateway == 'PayPal' ) {
				$content .= '<li>' . __( 'Zahlungsmethode: <strong>Dein PayPal-Konto</strong>', 'psts' ) . '</li>';
			} else if ( $old_gateway == 'Amazon' ) {
				$content .= '<li>' . __( 'Zahlungsmethode: <strong>Dein Amazon-Konto</strong>', 'psts' ) . '</li>';
			} else if ( $psts->get_expire( $blog_id ) >= 9999999999 ) {
				$content .= '<li>' . __( 'Ablaufdatum: <strong>Niemals</strong>', 'psts' ) . '</li>';
			} else {
				$content .= '<li>' . sprintf( __( 'Ablaufdatum: <strong>%s</strong>', 'psts' ), $end_date ) . '</li>';
			}

			$content .= '</ul><br />';
			if ( $old_gateway == 'PayPal' || $old_gateway == 'Amazon' ) {
				$content .= '<h3>' . __( 'Kündige Dein Abonnement', 'psts' ) . '</h3>';
				$content .= '<p>' . sprintf( __( 'Wenn Dein Abonnement noch aktiv ist, sollte Deine nächste geplante Zahlung %1$s betragen.', 'psts' ), $end_date ) . '</p>';
...
//		$content .= $psts->checkout_grid( $blog_id, $domain );
		...
		$content .= '<p><input id="psts-manual-submit" type="submit" name="psts_mp_submit" value="' . esc_attr__( 'Einreichen', 'psts' ) . '"></p>';
		$content .= '</div></form>';
	...
	public static function render_gateway( $domain, $blog_id, $args, $render_data = array(), $prefer_cc = false ) {
		global $psts;
		$content = '';
	...
	public static function process_checkout_form(  $domain, $blog_id, $process_data = array() ) {
		global $psts, $wpdb;
