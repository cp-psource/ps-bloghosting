...
class ProSites_Module_Plugins_Manager {
	...
	public static function get_description() {
		return __('Erstelle ganz einfach Plugin-Pakete, die nur für ausgewählte Bloghosting-Ebenen verfügbar sind. (Kann nicht mit "Premium Plugins" verwendet werden)', 'psts');
	}
...
//		}
	...
	function plug_network_page() {
		$module_page = add_submenu_page( 'psts', __( 'Bloghosting Premium Plugins Manager', 'psts' ), __( 'Premium Plugins Manager', 'psts' ), 'manage_network_options', 'psts-plugins-manager', array(
			&$this,
			'admin_page'
		) );
	...
	function admin_page() {
		...
		<div class="wrap">
			...
			<?php if( $updated ) { ?>
			<div class="updated">
				<p><?php _e( 'Einstellungen gespeichert!', 'psts' ); ?></p>
			</div>
			<?php } ?>
			<form method="post" action="">
				<div class="psts_level_plugins">
					<p><?php _e( 'Wähle Erweiterungen aus, die Du für diese Ebene verfügbar machen möchtest.', 'psts' ) ?></p>
					<table class="widefat prosites-premium-plugins">
						<thead>
							<tr>
								...
								<?php } ?>
								<th class="psts_plugin_manager_name"><?php _e( 'Erweiterung', 'psts' ) ?></th>
								<th class="psts_plugin_manager_version"><?php _e( 'Version', 'psts' ) ?></th>
								<th class="psts_plugin_manager_description"><?php _e( 'Beschreibung', 'psts' ) ?></th>
							</tr>
						</thead>
						<tbody id="plugins">
				...
				<p class="submit">
					<input type="submit" name="supporter_plugins_manager" class="button-primary" value="<?php _e( 'Änderungen speichern', 'psts' ) ?>"/>
				</p>
			</form>
		</div>

		<div class="current_setup">
			<h3>Das aktuelle Setup</h3>
			<table class="widefat prosites-premium-plugins">
				<thead>
					<tr>
						<th>Level Name</th>
	...
	function blog_options_form( $blog_id ) {
		...
		<p style="padding:5px 10px 0 10px;margin:0;">
			<?php _e( 'Die hier gewählten Erweiterungen sind für diese Seite zugänglich und überschreiben die Einstellungen für <a href="admin.php?page=psts-plugins">Premium-Plugins</a>. Deaktiviere diese Option, um zu diesen Berechtigungen zurückzukehren.', 'psts' ) ?>
		</p>
		...
		foreach ( $plugins as $file => $p ) {
			...
			<tr>
				<td>
					...
					echo '<label><input name="plugins[' . $file . ']" type="checkbox" value="1" ' . $checked . '/> ' . __( 'Aktivieren', 'psts' ) . '</label>';
					?>
				</td>
				<td><?php echo esc_html( $p['Name'] ); ?></td>
				<td><?php echo esc_html( $p['Version'] ); ?></td>
				<td><?php echo esc_html( $p['Autor'] ); ?></td>
			</tr>
		<?php
		}
		echo '</table>';
