...
class ProSites_Module_PremiumThemes {
	...
	function plug_network_page() {
		$page = add_submenu_page( 'psts', __( 'Bloghosting Premium Themes', 'psts' ), __( 'Premium Themes', 'psts' ), 'manage_network_options', 'psts-themes', array(
				&$this,
				'admin_page'
			) );
	}
...
<!--		<div class="postbox">-->
			...
			<div class="inside">
				<table class="form-table">
					<tr valign="top">
						<th scope="row" class="psts-help-div psts-rename-feature"><?php echo __( 'Funktion umbenennen', 'psts' ) . $psts->help_text( __( 'Erforderlich - Kein HTML! - Mach das kurz und bündig.', 'psts' ) ); ?></th>
						<td>
							<input type="text" name="psts[pt_name]" value="<?php echo esc_attr( $psts->get_setting( 'pt_name', __( 'Premium Themes', 'psts' ) ) ); ?>" size="30"/>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row" class="psts-help-div psts-theme-preview"><?php echo __( 'Themevorschau-Nachricht', 'psts' ) . $psts->help_text( __( 'Erforderlich - Kein HTML! - Diese Meldung wird angezeigt, wenn auf der Webseite der falschen Ebene eine Vorschau eines Premium-Themas angezeigt wird. "LEVEL" wird durch den erforderlichen Levelnamen für dieses Thema ersetzt.', 'psts' ) ); ?></th>
						<td>
							<input type="text" name="psts[pt_text]" value="<?php echo esc_attr( $psts->get_setting( 'pt_text', __( 'Aktualisiere auf LEVEL, um dieses Premium-Theme &raquo zu aktivieren;', 'psts' ) ) ); ?>" style="width: 95%"/>
						</td>
					</tr>
				</table>
			</div>
