...
/*
Plugin Name: Bloghosting (Feature: Zahlen um zu Bloggen)
*/
...
class ProSites_Module_PayToBlog {
	...
	public static function get_name() {
		return __( 'Zahlen um zu Bloggen', 'psts' );
	}

	// Module description for registering
	public static function get_description() {
		return __( 'Ermöglicht das vollständige Deaktivieren einer Seite sowohl im Front-End als auch im Back-End bis zur Bezahlung.', 'psts' );
	}
	...
	function __construct() {
		...
		self::$user_label       = __( 'Zahlen um zu Bloggen', 'psts' );
		self::$user_description = __( 'Seite deaktiviert, bis die Zahlung geklärt ist', 'psts' );
		...
		if ( $psts->get_setting( 'ptb_front_disable' ) && $disable_site ) {
			...
			if ( file_exists( WP_CONTENT_DIR . '/ptb-template.php' ) ) {
				...
				if ( is_user_logged_in() && current_user_can( 'edit_pages' ) ) {
					$content .= '<p><a href="' . $psts->checkout_url( $blog_id ) . '">' . __( 'Jetzt wieder aktivieren &raquo;', 'psts' ) . '</a></p>';
				}
				wp_die( $content );
			}
		}
	...
	function settings() {
		...
		<div class="inside">
			<table class="form-table">
				<tr valign="top">
					<th scope="row"
					    class="psts-help-div psts-checkout-message"><?php echo __( 'Checkout-Nachricht', 'psts' ) . $psts->help_text( __( 'Erforderlich - Diese Meldung wird auf der Checkout-Seite angezeigt, wenn die Seite nicht bezahlt wurde. HTML erlaubt', 'psts' ) ); ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Frontend deaktivieren', 'psts' ); ?></th>
					<td>
						<label><input type="checkbox" name="psts[ptb_front_disable]"
						              value="1"<?php checked( $psts->get_setting( 'ptb_front_disable' ) ); ?> /> <?php _e( 'Deaktivieren', 'psts' ); ?>
						</label></td>
				</tr>
				<tr valign="top">
					<th scope="row"
					    class="psts-help-div psts-frontend-restricted"><?php echo __( 'Front-End Eingeschränkt Nachricht', 'psts' ) . $psts->help_text( __( 'Erforderlich - Diese Meldung wird am Frontend der Seite angezeigt, wenn sie nicht bezahlt wurde und das Deaktivieren des Frontends aktiviert ist. HTML erlaubt', 'psts' ) ); ?></th>
					<td>
