...
/*
Plugin Name: Bloghosting (Feature: Beitrags/Seiten Kontingent)
*/
...
class ProSites_Module_PostingQuota {
	...
	public static function get_name() {
		return __( 'Beitrags/Seiten Kontingent', 'psts' );
	}

	// Module description for registering
	public static function get_description() {
		return __( 'Ermöglicht das Begrenzen der Anzahl der Beitragstypen für ausgewählte Bloghosting-Ebenen.', 'psts' );
	}

	function __construct() {
		global $psts;

		self::$user_label       = __( 'Beitrags/Seiten Kontingente', 'psts' );
		self::$user_description = __( 'Begrenzte Beitragstypen', 'psts' );
		...
		if ( is_array( $post_types ) ) {
			foreach ( $post_types as $post_type ) {
				...
				$defaults[ $post_type->name ] = array(
					'quota'   => 'unlimited',
					'message' => sprintf( __( "Du hast das Veröffentlichungslimit erreicht. Um mehr %s zu veröffentlichen, aktualisiere bitte auf LEVEL &raquo;", 'psts' ), $post_type->label ),
				);
			}
		}
	...
	function settings() {
		...
		<div class="inside">
			<table class="form-table post-page-quota">
				<tr valign="top">
					<th scope="row" class="pro-site-level psts-quota-prosite-level"><?php echo __( 'Festlegen der Kontingente pro Ebene', 'psts' ) . $psts->help_text( __( 'Wähle aus ob Sie Kontingente pro Ebene in Deinem Netzwerk festlegen möchtest. Wenn aktiviert, hat jedes Level unterschiedliche Quoten', 'psts' ) ); ?></th>
					<td>
						<input type="radio" name="psts[per_level]" class="per_level" value="1" <?php checked( $per_level, 1 ); ?> /><?php echo __( 'Ja', 'psts' ); ?>
						<br/>
						<input type="radio" name="psts[per_level]" class="per_level" value="0" <?php checked( $per_level, 0 ); ?> /><?php echo __( 'Nein', 'psts' ); ?>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row" class="pro-site-level psts-quota-prosite-level">
						<?php
						$label_description = ( $per_level == 1 ) ? __( 'Wähle eine Ebene aus, um deren Kontingente zu definieren', 'psts' ) : __( 'Wähle die Mindeststufe aus, die zum Entfernen von Kontingenten erforderlich ist', 'psts' );
						echo __( 'Bloghosting Level', 'psts' ) . $psts->help_text( $label_description );
						?>
					</th>
					<td>
						<select name="psts[pq_level]" id="pq_level" class="chosen">
							<option value="0" <?php selected( $selected_level, 0 ); ?>><?php _e( 'Standard - Basis Stufe', 'psts' ); ?></option>
							<?php
				...
				if ( is_array( $post_types ) ) {
					foreach ( $post_types as $post_type ) {
						...
						$quota_msg = isset( $quota_settings[ $post_type->name ]['message'] ) ? $quota_settings[ $post_type->name ]['message'] : sprintf( __( "Du hast das Veröffentlichungslimit erreicht. Um mehr %s zu veröffentlichen, aktualisiere bitte auf LEVEL &raquo;", 'psts' ), $post_type->label );
						?>
						<tr valign="top">
							<th scope="row"><?php printf( __( '%s Kontingent', 'psts' ), $post_type->label ); ?></th>
						</tr>
						<tr>
							<td><?php printf( __( 'Veröffentlichungslimit', 'psts' ), $post_type->label ); ?></td>
							<td>
								<select name="psts[pq_quotas][<?php echo $post_type->name; ?>][quota]" class="chosen">
									<option value="unlimited"<?php selected( $quota, 'unlimited' ); ?>><?php _e( 'Unbegrenzt', 'psts' ); ?></option>
									<?php
						...
						<tr>
							<td class="upgrade-message"><?php echo __( 'Upgrade-Nachricht', 'psts' ) . '<img width="16" height="16" src="' . $psts->plugin_url . 'images/help.png" class="help_tip"><div class="psts-help-text-wrapper period-desc"><div class="psts-help-arrow-wrapper"><div class="psts-help-arrow"></div></div><div class="psts-help-text">' . __( 'Wird auf dem jeweiligen Bildschirm zum Hinzufügen von Posts, Seiten oder Medien für Webseiten angezeigt, die ihr Kontingent aufgebraucht haben. "LEVEL" wird durch den erforderlichen Levelnamen ersetzt', 'psts' ) . '</div></div>'; ?></td>
							<td>
				...
				?>
				<tr valign="top">
					<th scope="row"><?php echo __( 'Hinweis auf höchster Ebene', 'psts' ); ?></th>
				</tr>
				<tr>
					<td class="upgrade-message">
						<?php echo __( 'Nachricht', 'psts' ) . '<img width="16" height="16" src="' . $psts->plugin_url . 'images/help.png" class="help_tip"><div class="psts-help-text-wrapper period-desc"><div class="psts-help-arrow-wrapper"><div class="psts-help-arrow"></div></div><div class="psts-help-text">' . __( 'Wird angezeigt, wenn Du Grenzwerte für die höchste Stufe festgelegt hast und diese Grenzwerte erreicht wurden. Alle Webseitenbesitzer auf höchster Ebene, die ihre Grenzen erreicht haben, sehen diese Meldung.', 'psts' ) . '</div></div>'; ?></td>
					<td>
						<?php
						$message = isset( $quota_settings['highest_level_message'] ) ? $quota_settings['highest_level_message'] : __( 'Du hast Deine Veröffentlichungsgrenzen erreicht, keine Upgrades für diesen Level. Wende Dich an den Administrator. &raquo;', 'psts' );
						?>
	...
	function message() {
		...
		if ( is_numeric( $limit ) && wp_count_posts( $post_type )->publish >= $limit ) {
			...
			if ( $level < count( $levels ) ) {
				...
				$notice = isset( $quota_settings['highest_level_message'] ) ? $quota_settings['highest_level_message'] : "Du hast Deine Veröffentlichungsgrenzen erreicht, keine Upgrades für diese Stufe. Wende Dich an den Administrator. &raquo;";
	...
	public function remove_publish_option() {
		...
		if ( $exceeded && get_post()->post_status != 'publish' ) {
			...
			<a id="psts-upgrade" class="button button-primary button-large" href="<?php echo $psts->checkout_url( get_current_blog_id() ); ?>"><?php _e( 'Upgrade Dein Konto', 'psts' ); ?></a>
			</div><?php
		}
	}
	...
	public function include_text( $level ) {
		...
		$text = "<ul>" . __( "Veröffentlichungslimits: ", 'psts' );
	...
	function limit_media_upload( $file ) {
		...
		if ( ! $this->media_upload_exceeded( $blog_id ) ) {
			return $file;
		} else {
			$levels = get_site_option( 'psts_levels' );
			if ( count( $levels ) > $level ) {
				...
				$level += 1;
				$name    = $psts->get_level_setting( $level, 'name' );
				$message = ! empty( $quota_settings['attachment']['message'] ) ? $quota_settings['attachment']['message'] : __( "Du hast das Veröffentlichungslimit erreicht. Um weitere Medien zu veröffentlichen, aktualisiere bitte auf LEVEL »", 'psts' );
				if ( ! empty( $name ) ) {
					$message = str_replace( 'LEVEL', $name, $message );
				}
			} elseif ( count( $levels ) == $level ) {
				$message = isset( $quota_settings['highest_level_message'] ) ? $quota_settings['highest_level_message'] : "Du hast Deine Medienveröffentlichungsgrenzen erreicht. Wende Dich an den Administrator, wenn Du weitere Medien veröffentlichen möchtest. &raquo;";
			}
			$file['error'] = $message;
		}
	 ...
	 */
	function media_upload_exceeded( $blog_id ) {

		$limit = $this->limit( $blog_id );
