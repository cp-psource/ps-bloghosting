...
/*
Plugin Name: Bloghosting (Feature: Bulk Upgrades)
*/
...
class ProSites_Module_BulkUpgrades {
	...
	public static function get_description() {
		return __( 'Ermöglicht den Verkauf von Upgrades auf Bloghostinge-Ebene in Massenpaketen.', 'psts' );
	}
	...
	public static function run_critical_tasks() {
		...
		self::$user_description = __( 'Kann in Massenpaketen aktualisiert werden', 'psts' );
	}
	...
	public static function ipn_handler() {
		...
		if ( ! isset( $_POST['payment_status'] ) ) {
			...
			$args['user-agent'] = "Bloghosting/{$psts->version}: https://n3rds.work/piestingtal_source/ps-bloghosting-multisite-next-level-plugin/ | PayPal Bulk Upgrades/{$psts->version}";
			$args['body']       = $req;
			...
			if ( is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) != 200 || $response['body'] != 'VERIFIED' ) {
				...
				_e( 'Beim Überprüfen der IPN-Zeichenfolge mit PayPal ist ein Problem aufgetreten. Bitte versuche es erneut.', 'psts' );
				exit;
			}
			...
			switch ( $_POST['payment_status'] ) {
				...
				case 'Reversed':
					// case: charge back
					$note = __( 'Die letzte Transaktion wurde rückgängig gemacht. Grund: Zahlung wurde storniert (Rückbelastung)', 'psts' );
					list( $bid, $uid, $credits, $amount, $currency, $stamp ) = explode( '_', $_POST['custom'] );
				...
				case 'Refunded':
					// case: refund
					$note = __( 'Die letzte Transaktion wurde rückgängig gemacht. Grund: Die Zahlung wurde zurückerstattet', 'psts' );
					list( $bid, $uid, $credits, $amount, $currency, $stamp ) = explode( '_', $_POST['custom'] );
					//supporter_insert_update_transaction($bid, $_POST['parent_txn_id'], $_POST['payment_type'], $stamp, $amount, $currency, $_POST['payment_status']);
					self::debit_credits( $uid, $credits );
					self::update_note( $uid, $note );
				...
				case 'Denied':
					// case: denied
					$note = __( 'Die letzte Transaktion wurde rückgängig gemacht. Grund: Zahlung verweigert', 'psts' );
					list( $bid, $uid, $credits, $amount, $currency, $stamp ) = explode( '_', $_POST['custom'] );
				...
				case 'Pending':
					...
					$pending_str = array(
						'address'        => __( 'Der Kunde hat keine bestätigte Lieferadresse angegeben', 'psts' ),
						'authorization'  => __( 'Zahlung noch nicht erfasst', 'psts' ),
						'echeck'         => __( 'eCheck, das noch nicht gelöscht wurde', 'psts' ),
						'intl'           => __( 'Zahlung wartet auf Genehmigung durch Dienstleister', 'psts' ),
						'multi-currency' => __( 'Zahlung wartet darauf, dass der Dienstanbieter den Prozess mit mehreren Währungen abwickelt', 'psts' ),
						'unilateral'     => __( 'Der Kunde hat seine E-Mail noch nicht registriert oder bestätigt', 'psts' ),
						'upgrade'        => __( 'Warten auf das Upgrade des PayPal-Kontos durch den Dienstanbieter', 'psts' ),
						'verify'         => __( 'Warten, bis der Dienstanbieter sein PayPal-Konto überprüft hat', 'psts' ),
						'*'              => ''
					);
					$reason      = @$_POST['pending_reason'];
					$note        = __( 'Letzte Transaktion steht noch aus. Grund:', 'psts' ) . ( isset( $pending_str[ $reason ] ) ? $pending_str[ $reason ] : $pending_str['*'] );
					list( $bid, $uid, $credits, $amount, $currency, $stamp ) = explode( '_', $_POST['custom'] );
					//supporter_insert_update_transaction($bid, $_POST['txn_id'], $_POST['payment_type'], $stamp, $amount, $currency, $_POST['payment_status']);
					self::update_note( $uid, $note );
					break;
	...
	public static function user_profile_update() {
		...
		if ( is_super_admin() && isset( $_POST['psts_credits'] ) ) {
			...
			self::update_note( $user_id, sprintf( __( '%s Bulk-Upgrade-Guthaben wurden von einem Administrator manuell zu Deinem Konto hinzugefügt.', 'psts' ), intval( $_POST['psts_credits'] ) ) );
		}
	}
	...
	public static function user_profile_fields() {
		...
		?>
		<h3><?php _e( 'Bloghosting Bulk Upgrade Guthaben', 'psts' ); ?></h3>
		<table class="form-table">
			<tr>
				<th align="right"><?php _e( 'Aktuelles Guthaben:', 'psts' ); ?></th>
				<td><?php printf( __( 'Dieser Benutzer hat %s Upgrade-Guthaben auf seinem Konto.', 'psts' ), '<strong>' . number_format_i18n( self::get_credits( $user_id ) ) . '</strong>' ) . '</p>'; ?></td>
			</tr>
			<tr>
				<th align="right">
					<label for="psts_bulk_credits"><?php _e( 'Manuelles Erteilen von Guthaben:', 'psts' ); ?></label></th>
				<td>
	...
	function settings() {
		...
		<!--			<span class="description">--><?php //_e( 'Ermöglicht den Verkauf von Upgrades auf Bloghostinge-Ebene in Massenpaketen.', 'psts' ) ?><!--</span>-->
		<!--		</h3>-->

		<div class="inside">
			<?php if ( get_site_option( "supporter_bulk_upgrades_paypal_payment_type" ) == 'recurring' ) { ?>
				<p><?php _e( 'Wichtig - Wenn Du zuvor Massen-Upgrades und Abonnements mit Supporter 2.x verwendet hast, musst Du die Dateien <em>/pro-sites/pro-sites-files/gateways/backwards-compatibility/<strong>supporter-bulk-upgrades-paypal.php</strong></em>auf den Webroot dieser Webseite übertragen, um Probleme mit Zahlungen aus vorhandenen Abonnements zu vermeiden.', 'psts' ) ?></p>
			<?php } ?>
			<table class="form-table">
				<tr>
				...
				<tr valign="top">
					...
					<td><select name="psts[bu_status]" class="chosen">
							<option
								value="live"<?php selected( $psts->get_setting( 'bu_status' ), 'live' ); ?>><?php _e( 'Live Seite', 'psts' ) ?></option>
							<option
								value="test"<?php selected( $psts->get_setting( 'bu_status' ), 'test' ); ?>><?php _e( 'Test Modus (Sandbox)', 'psts' ) ?></option>
						</select>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"
						class="psts-help-div psts-bulk-payment-type"><?php echo __( 'PayPal Zahlungsart', 'psts' ) . $psts->help_text( __( 'Wiederkehrend = PayPal 12-Monats-Abonnement', 'psts' ) ); ?></th>
					<td><select name="psts[bu_payment_type]" class="chosen">
							...
							<option
								value="recurring"<?php selected( $psts->get_setting( 'bu_payment_type' ), 'recurring' ); ?>><?php _e( 'Wiederkehrend', 'psts' ) ?></option>
						</select>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"
						class="psts-help-div psts-credit-level"><?php echo __( 'Guthabenn Level', 'psts' ) . $psts->help_text( __( 'Auf welche Bloghosting Level Guthaben wird ein Upgrade durchgeführt?.', 'psts' ) ); ?></th>
					<td>
