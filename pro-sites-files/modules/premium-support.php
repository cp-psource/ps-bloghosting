...
/*
Plugin Name: Bloghosting (Feature: Premium Support)
*/
...
class ProSites_Module_Support {
	...
	public static function get_description() {
		return __('Ermöglicht die Bereitstellung einer Premium-Support-Seite direkt für E-Mails für ausgewählte Bloghosting-Level.', 'psts');
	}
...
//		add_action( 'psts_settings_page', array( &$this, 'settings' ) );
		...
		self::$user_description = __( 'Schließe Premium direkt an den E-Mail-Support ein', 'psts' );
	}
...
<!--			</h3>-->
			...
			<div class="inside">
				<table class="form-table">
					<tr valign="top">
						<th scope="row" class="psts-help-div psts-premium-prosite-level"><?php echo __( 'Bloghosting Level', 'psts' ) . $psts->help_text( __( 'Wähle die Mindeststufe aus, die für die Nutzung des Premium-Supports erforderlich ist.', 'psts' ) ); ?></th>
						<td>
					...
					<tr valign="top">
						<th scope="row" class="psts-help-div psts-support-email"><?php echo __( 'Support Email', 'psts' ) . $psts->help_text( __( 'Die E-Mail-Adresse, an die Premium-Supportnachrichten gesendet werden sollen.', 'psts' ) ); ?></th>
						<td>
							<input type="text" name="psts[ps_email]" id="ps_email" value="<?php echo esc_attr( $psts->get_setting( 'ps_email' ) ); ?>" size="40"/>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row" class="psts-help-div psts-rename-feature"><?php echo __( 'Feature umbenennen', 'psts' ) . $psts->help_text( __( 'Erforderlich - Kein HTML! - Mach das kurz und bündig.', 'psts' ) ); ?></th>
						<td>
							<input type="text" name="psts[ps_name]" id="ps_name" value="<?php echo esc_attr( $psts->get_setting( 'ps_name' ) ); ?>" size="30"/>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row" class="psts-help-div psts-premium-support"><?php echo __( 'Premium Support Hinweis', 'psts' ) . $psts->help_text( __( 'Die Nachricht, die auf der Premium Support-Seite angezeigt wird. HTML erlaubt.', 'psts' ) ); ?></th>
						<td>
					...
					<tr valign="top">
						<th scope="row" class="psts-help-div psts-restricted-message"><?php echo __( 'Eingeschränkt Hinweis', 'psts' ) . $psts->help_text( __( 'Erforderlich - Diese Meldung wird auf Webseiten angezeigt, die keine Berechtigungen haben. "LEVEL" wird durch den erforderlichen Levelnamen ersetzt.', 'psts' ) ); ?></th>
						<td>
	...
	function support_page() {
		...
		<div class="wrap">
			...
			if ( isset( $_POST['support-message'] ) && is_supporter() ) {
				...
				$subject         = sprintf( __( 'Premium Support Anfrage: %s', 'psts' ), get_bloginfo( 'url' ) );
			...
			<form method="post" action="">
				...
				<p class="submit">
					<input type="submit" value="<?php _e( 'Anfrage einreichen &raquo;', 'psts' ) ?>"<?php echo $disabled; ?> />
				</p>
			</form>
		</div>
	<?php
