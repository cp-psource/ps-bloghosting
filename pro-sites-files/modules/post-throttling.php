...
class ProSites_Module_PostThrottling {
	...
	public static function get_name() {
		return __( 'Beitragsdrosselung', 'psts' );
	}

	// Module description for registering
	public static function get_description() {
		return __( 'Ermöglicht es die Anzahl der Beiträge zu begrenzen, die täglich/stündlich pro Seite veröffentlicht werden dürfen.', 'psts' );
	}
	...
	public function __construct() {
		...
		self::$user_label       = __( 'Beitragsdrosselungg', 'psts' );
		self::$user_description = __( 'Begrenze die Veröffentlichungsrate für Beiträge.', 'psts' );
	}
	...
	private static function _getThrottlingPeriods() {
		return array(
			self::PERIOD_HOURLY => array(
				'label' => __( 'Stundenlimits', 'psts' ),
				'frame' => HOUR_IN_SECONDS,
			),
			self::PERIOD_DAILY  => array(
				'label' => __( 'Tageslimits', 'psts' ),
	...
	public function renderLimitsInformation() {
		...
		if ( $exceeded && get_post()->post_status != 'publish' ) {
			...
			<a id="psts-upgrade" class="button button-primary button-large"
				href="<?php echo $psts->checkout_url( get_current_blog_id() ); ?>"><?php _e( 'Upgrade Dein Konto', 'psts' ); ?></a>
			</div><?php
		}
	}
	...
	public function renderModuleSettings() {
		...
		<div class="inside">
			<p><?php echo __( 'Mit dem Beitragsdrosselung-Modul kannst Du die Anzahl der täglich oder stündlich veröffentlichten Beiträge pro Seite begrenzen, um ein Überfluten mit Beiträgen durch eine einzelne Seite zu vermeiden.', 'psts' ), ' '; ?></p>
			...
			<td>
				...
				<div class="plan-th-limits">
					<select name="<?php echo $key . '[0]'; ?>" class="chosen">
						...
						<option
							value="unlimited"<?php selected( $free_value, 'unlimited' ); ?>><?php _e( 'Unbegrenzt', 'psts' ); ?></option>
						<?php
				...
				<td>
					<div class="plan-th-limits">
						<select name="<?php echo $key . '[' . $level . ']'; ?>" class="chosen">
							<option
								value="unlimited"<?php selected( $value, 'unlimited' ); ?>><?php _e( 'Unbegrenzt', 'psts' ); ?></option>
							<?php
							for ( $counter = 1; $counter <= 1000; $counter ++ ) {
	...
	function message() {
		...
		if ( in_array( $current_screen->id, array( $post_type, 'edit-' . $post_type ) ) ) {
			...
			if ( $exceeded ) {
				$period_human_form = 'throttling_hour' == $key ? __( 'stündlich', 'psts' ) : __( 'täglich', 'psts' );
				$upgrade_message   = is_super_admin( $blog_id ) ? 'Du kannst <a href="' . $psts->checkout_url( $blog_id ) . '">' . __( 'Upgraden', 'psts' ) . '</a> um weitere Beiträge zu veröffentlichen. Wende Dich an den Seiten-Administrator, um weitere Informationen zu erhalten' : __( 'Wende Dich an den Seiten-Administrator, um weitere Informationen zu erhalten' );
