...
/*
Plugin Name: Bloghosting (Modul: Upload Speicherplatz)
*/
...
class ProSites_Module_Quota {
	...
	public static function get_name() {
		return __( 'Upload Speicherplatz', 'psts' );
	}

	// Module description for registering
	public static function get_description() {
		return __( 'Ermöglicht es Dir, Bloghosting Webseiten zusätzlichen Upload-Speicherplatz zuzuweisen.', 'psts' );
	}
	...
	function __construct() {
		...
		self::$user_label       = __( 'Kontingent', 'psts' );
		self::$user_description = __( 'Upload Kontingent', 'psts' );

		//add checkout grid text
		$this->checkout_name = __( 'Upload Speicherplatz', 'psts' ); //can only i18n in _construct()
		$this->checkout_desc = __( 'Erhalte zusätzlichen Upload-Speicherplatz für Deine Bilder, Medien und anderen Dateien.', 'psts' ); //can only i18n in _construct()
	...
	function settings() {
		...
		<div class="inside">
			<table class="form-table">
				<tr valign="top">
					<th scope="row"
					    class="psts-help-div psts-quota-amounts"><?php echo __( 'Speicherplaz Kontingente', 'psts' ) . $psts->help_text( __( 'Jedes Level sollte ein identisches oder zunehmend höheres Kontingent haben.', 'psts' ) ); ?></th>
					<td><?php
						if ( function_exists( 'psts_ads_upgrade_active' ) && psts_ads_upgrade_active() ) {
							...
							echo ' ' . $level . ' - ' . __( 'Anzeigen entfernt (aktualisiert)', 'psts' ) . '</label><br />';
						}
				...
				<tr valign="top">
					<th scope="row"
					    class="psts-help-div psts-quota-message"><?php echo __( 'Kontingentnachricht', 'psts' ) . $psts->help_text( __( 'Erforderlich - Diese Nachricht wird im Dashboard und im Formular zum Hochladen von Medien als Werbung für das Upgrade auf die nächste Ebene angezeigt. "LEVEL" wird durch den erforderlichen Ebenennamen ersetzt, und "SPACE" wird durch den zusätzlichen Upload-Speicherplatz in der nächsten Ebene ersetzt.', 'psts' ) ); ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"
					    class="psts-help-div psts-out-of-space"><?php echo __( 'Kontingent erreicht Hinweis', 'psts' ) . $psts->help_text( __( 'Erforderlich - Diese Meldung wird im Dashboard angezeigt, wenn kein Upload-Speicherplatz mehr vorhanden ist. "LEVEL" wird durch den erforderlichen Ebenennamen ersetzt, und "SPACE" wird durch den zusätzlichen Upload-Speicherplatz in der nächsten Ebene ersetzt.', 'psts' ) ); ?></th>
					<td>
			...
			<p>
				<span
					class="description"><?php _e( 'HINWEIS: Wenn Du in der Lage sein musst, das Upload-Kontingent beim Bearbeiten der Webseite pro Webseite zu überschreiben, füge dies Deiner Datei wp-config.php hinzu:', 'psts' ) ?>
	...
	public function add_quota_to_library() {
		...
		/* translators: 1: number of megabytes, 2: percentage */
			__( '%1$s MB (%2$s%%) von %3$s MB belegt.', 'psts' ),
