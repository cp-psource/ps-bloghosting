...
class ProSites_Module_Plugins {
	...
	public static function get_description() {
		return __('Ermöglicht das Erstellen von Plugin-Paketen, die nur für ausgewählte Bloghosting-Ebenen verfügbar sind. (Kann nicht mit "Premium Plugins Manager" verwendet werden)', 'psts');
	}
...
//			add_filter( 'site_option_menu_items', array( get_class(), 'enable_plugins_page' ) );
	...
	function __construct() {
		...
		self::$user_description = __( 'Füge Premium-Plugins hinzu', 'psts' );
	...
	function plug_network_page() {
		$module_page = add_submenu_page( 'psts', __( 'Bloghosting Premium Plugins', 'psts' ), __( 'Premium Plugins', 'psts' ), 'manage_network_options', 'psts-plugins', array( &$this, 'admin_page' ) );
	...
	function action_links( $action_links, $plugin_file, $plugin_data, $context ) {
		...
		if ( isset( $psts_plugins[ $plugin_file ]['level'] ) && $psts_plugins[ $plugin_file ]['level'] != 0 ) {
			$rebrand = sprintf( __( 'Nur für %s', 'psts' ), $psts->get_level_setting( $psts_plugins[ $plugin_file ]['level'], 'name' ) );
			$color   = 'green';
	...
	function message_output() {
		...
		if ( $pagenow == 'plugins.php' && is_super_admin() ) {
			echo '<div class="updated"><p>' . __( 'Als Super Admin kannst Du beliebige Plugins für diese Seite aktivieren.', 'psts' ) . '</p></div>';
		}

		//Warns of Multisite Plugin Manager conflict
		if ( class_exists( 'PluginManager' ) && is_super_admin() ) {
			echo '<div class="error"><p>' . __( 'WARNUNG: Der Multisite Plugin Manager und das Premium Plugins-Modul sind nicht kompatibel. Bitte deaktiviere den Multisite Plugin Manager.', 'psts' ) . '</p></div>';
		}
	}
	...
	function blog_options_form( $blog_id ) {
		...
		<h3><?php _e( 'Optionen zum Überschreiben von Plugins', 'psts' ) ?></h3>
		<p style="padding:5px 10px 0 10px;margin:0;">
			<?php _e( 'Die hier gewälten Plugins sind für diese Seite zugänglich und überschreiben die Einstellungen für <a href="admin.php?page=psts-plugins">Premium-Plugins</a>. Deaktiviere diese Option, um zu diesen Berechtigungen zurückzukehren.', 'psts' ) ?>
		</p>
		<table class="widefat" style="margin:10px;width:95%;">
		<thead>
		<tr>
			<th title="<?php _e( 'Seiten-Benutzer können aktivieren/deaktivieren', 'psts' ) ?>"><?php _e( 'Nutzerkontrolle', 'psts' ) ?></th>
			<th><?php _e( 'Name', 'psts' ); ?></th>
			<th><?php _e( 'Version', 'psts' ); ?></th>
			<th><?php _e( 'Autor', 'psts' ); ?></th>
		</tr>
		</thead>
		<?php
		foreach ( $plugins as $file => $p ) {
			...
			<tr>
				<td>
					...
					echo '<label><input name="plugins[' . $file . ']" type="checkbox" value="1" ' . $checked . '/> ' . __( 'Aktiviere', 'psts' ) . '</label>';
					?>
				</td>
...
<!--		<div class="postbox">-->
			...
			<span class="description"><?php _e( 'Ermöglicht das Erstellen von Plugin-Paketen, die nur für ausgewählte Bloghosting-Ebenen verfügbar sind.', 'psts' ) ?></span>

			<div class="inside">
				<table class="form-table">
					<tr valign="top">
						<th scope="row" class="psts-help-div psts-pplugin-rename-feature"><?php echo __( 'Funktion umbenennen', 'psts' ) . $psts->help_text( __( 'Erforderlich - Kein HTML! - Mach das kurz und süß.', 'psts' ) ); ?></th>
						<td>
...
<!--		</div>-->
	...
	function admin_page() {
		...
		<div class="wrap">
			...
			<p><?php _e( 'Wähle die minimale Bloghosting-Ebene für Premium-Plugins aus, die Du für Webseiten dieser Stufe oder höher aktivieren möchtest. Wenn Du "Keine" auswählst, ist das Plugin nur für Superadministratoren verfügbar. Durch Aktivieren von Automatisch aktivieren wird das Plugin aktiviert, wenn ein Upgrade auf diese Ebene durchgeführt wird. Nur Netzwerk- und netzwerkaktivierte Plugins werden in dieser Liste nicht angezeigt. Beachte dass Du Plugin-Berechtigungen auch pro Seite auf der Seite <a href="sites.php">Seiten bearbeiten</a> überschreiben kannst.', 'psts' ); ?></p>
			...
			<form method="post" action="">
				...
				<p class="submit">
					<input type="submit" name="supporter_plugins" class="button-primary" value="<?php _e( 'Änderungen speichern', 'psts' ) ?>"/>
				</p>
			</form>
		</div>
	<?php
